...
router.post('/book/:studentId', (req: Request, res: Response) => {
  ...
  if (!slot || slot.status !== '受付中' || slot.reserved_count >= slot.max_students) {
    db.close(); res.redirect(`/reservations/book/${req.params.studentId}?from=${date}&error=満席または受付終了`); return;
  }
  ...
  if (dup) {
    db.close(); res.redirect(`/reservations/book/${req.params.studentId}?from=${date}&error=同時間帯にすでに予約があります`); return;
  }
